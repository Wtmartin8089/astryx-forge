 ...
 *   Types & Config
 *   ========================= */
type StardateModel = "TNG" | "CUSTOM";
type Theme = "DEFAULT" | "LCARS";

interface CustomParams {
  baseYear: number;     // e.g., 2323 for TNG-like
  unitsPerYear: number; // e.g., 1000 for TNG-like
  offset?: number;      // fixed shift after scaling
}
...
function fmtDateForInput(date: Date): string {
...
function parseInputToUTC(v: string): Date | null {
  if (!v) return null;
...
function forceYearKeepTime(d: Date, year: number) {
  return new Date(
    Date.UTC(
      year,
      d.getUTCMonth(),
...
function setDateISO(setter: (s: string) => void, d: Date) {
...
function toTNGStardate(date: Date): number {
  const Y = date.getUTCFullYear();
...
function fromTNGStardate(sd: number): Date {
...
// Custom linear model
function toCustomStardate(date: Date, p: CustomParams): number {
  const Y = date.getUTCFullYear();
  const doy = dayOfYear(date);
  const days = isLeap(Y) ? 366 : 365;
  const sec =
...
function fromCustomStardate(sd: number, p: CustomParams): Date {
 ...
 *   Component
 *   ========================= */
export default function StardateWidget({
  theme = "LCARS",
}: {
  theme?: Theme;
}) {
  ...
  const [model, setModel] = useState<StardateModel>(
    () => (url.searchParams.get("model") as StardateModel) || "TNG"
  );
  ...
  const [dateInput, setDateInput] = useState<string>(() => {
    ...
    if (q) {
      ...
      if (d) return fmtDateForInput(d);
    }
    return fmtDateForInput(new Date());
  });
  const [sdInput, setSdInput] = useState<string>(
    () => url.searchParams.get("sd") || ""
  );

  const dateObj = useMemo(
    () => parseInputToUTC(dateInput) ?? new Date(),
                          [dateInput]
  );
   ...
   *     ========================= */
  return (
    <div className={`min-h-screen w-full ${shell ? lcars.bgShell : "bg-black"} text-gray-100 p-4 md:p-6`}>
    ...
    {/* PRESETS */}
          <div className="flex flex-wrap items-center gap-2">
            <button
              className={`${lcars.btn} ${lcars.a}`}
              onClick={() => setDateISO(setDateInput, forceYearKeepTime(new Date(), 2364))}
            >
              TNG Now (2364)
            </button>
          <button
              className={`${lcars.btn} ${lcars.b}`}
              onClick={() => setDateISO(setDateInput, forceYearKeepTime(new Date(), 2369))}
            >
              DS9 S1 (2369)
            </button>
            <button
              className={`${lcars.btn} ${lcars.c}`}
              onClick={() => setDateISO(setDateInput, forceYearKeepTime(new Date(), 2371))}
            >
              VOY S1 (2371)
            </button>
            ...
            <div className={`${lcars.card} p-5 space-y-3`}>
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <button
                    className={`ml-auto ${lcars.btn} ${lcars.b}`}
                    onClick={() => computedDate && copy(fmtDateForInput(computedDate))}
                  >
                    Copy
                  </button>
                </div>
          ...
          <section className={`${lcars.card} p-5 space-y-3`}>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                value={model}
                onChange={(e) => setModel(e.target.value as StardateModel)}
              >
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    className={`w-full ${lcars.input}`}
                    value={customParams.baseYear}
                    onChange={(e) =>
                      setCustomParams((p) => ({ ...p, baseYear: Number(e.target.value) }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className={`w-full ${lcars.input}`}
                    value={customParams.unitsPerYear}
                    onChange={(e) =>
                      setCustomParams((p) => ({ ...p, unitsPerYear: Number(e.target.value) }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    className={`w-full ${lcars.input}`}
                    value={customParams.offset ?? 0}
                    onChange={(e) =>
                      setCustomParams((p) => ({ ...p, offset: Number(e.target.value) }))
                    }
                  />
                </div>
              </div>
